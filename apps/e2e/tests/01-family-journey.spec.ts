/**
 * The scenario: one household signs up, builds a family of three, and — once the
 * billing phase is re-enabled — pays for the «العائلة» plan in LemonSqueezy test
 * mode and sees it reflected in the account.
 *
 * Two phases, one continuous account:
 *
 *   1. HOUSEHOLD — signup through the real form, the seeded trial, mom's profile,
 *      the husband, the child, the housekeeper, and the tier gate refusing a
 *      family of three on a starter trial. Needs nothing but a Supabase target.
 *
 *   2. PAYMENT (@billing) — checkout, webhook activation, and the paid state.
 *      DEFERRED by default; every test carries the tag and the default run
 *      filters it out. Nothing is commented out or deleted — re-enable the whole
 *      phase with E2E_INCLUDE_BILLING=1.
 *
...
let closeApiContext: (() => Promise<void>) | undefined;

test.describe.configure({ mode: "serial" });

// File-level, so the signed-in context survives into the payment phase when it
// is enabled and is still released when it is not.
test.afterAll(async () => {
...
test.describe("Family of three — signup and household", () => {
  ...
  test("the household renders on the family page", async () => {
    verifies(
      "The customer-visible output: /family shows the husband, the child and the " +
        "housekeeper by name — independently of any payment state.",
    );
...
// ── Phase 2: payment (deferred) ──────────────────────────────────────────────

test.describe("Family of three — payment and paid state", () => {
  test(
    "checkout mints a LemonSqueezy TEST-MODE session for the family plan",
    { tag: BILLING_TAG },
    async ({ cfg }) => {
      verifies(
  ...
  test(
    "a signed LemonSqueezy webhook activates the subscription",
    { tag: BILLING_TAG },
    async ({ cfg }) => {
      verifies(
      ...
      const res = await postWebhook(apiContext, payload, requireWebhookSecret(cfg));
      expect(
        res.status(),
  ...
  test(
    "a payment_success invoice event keeps the subscription active",
    { tag: BILLING_TAG },
    async ({ cfg }) => {
      verifies(
  ...
  test(
    "the account shows an active «العائلة» monthly subscription",
    { tag: BILLING_TAG },
    async () => {
      verifies(
  ...
  test("the subscription status API agrees", { tag: BILLING_TAG }, async () => {
    verifies(
  ...
  test("the amount billed is the family plan's price", { tag: BILLING_TAG }, async ({ cfg }) => {
    verifies(
