...
1. **[DONE — owner applied 00023, 07/2026]** Verdict-clearing («كيف كانت؟» un-tap) now actually deletes instead of silently no-opping. Applying it also **proves 00017 is applied**, since 00023 alters `meal_verdicts` and would otherwise have errored on a missing relation — so CLAUDE.md's "00017 NOT YET APPLIED" line is stale. **Follow-up: confirm the rest of the 00015–00022 chain with `scripts/verify-migrations.sql`, correct the CLAUDE.md baseline, and regenerate `database.types.ts` (`pnpm --filter @fitlife/app db:types`) for every applied migration** — until the types are regenerated the engagement code still reads those tables through untyped `as unknown as SupabaseClient` casts, which compile against a schema the database has since moved past.
