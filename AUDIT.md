...
1. **[DONE — owner applied 00015–00023, 07/2026]** Verdict-clearing («كيف كانت؟» un-tap) now actually deletes instead of silently no-opping, and the CLAUDE.md migration baseline has been corrected (six "NOT YET APPLIED" markers; that language is now labelled historical).

   **Open follow-up — `database.types.ts` is one batch behind prod.** Inspected directly: the generated types cover 00014 and 00017 (`plan_generations.plan_kind`, `meal_checkins.local_date`, `meal_verdicts.canonical_key`, `body_logs`, `member_exceptions`) but NOT 00018–00022 — `meal_checkins.member_id`, `body_logs.photo_path`, and the `workout_checkins` and `meal_absences` tables are all absent. That gap is exactly why the engagement layer reads through `as unknown as SupabaseClient` casts and `select("*")`: those were written to compile against types that did not know the columns. Now that prod HAS them, the casts are the only thing left, and they silently disable type-checking on `member_id`, `local_date`, `intensity` and `photo_path` across every engagement write.

   **The casts CANNOT be removed until the types are regenerated** — removing them today fails `tsc`, because the types still lack the columns. Regeneration needs `pnpm --filter @fitlife/app db:types` (`supabase gen types typescript --local`) run against a local DB migrated to 00023; it cannot run in CI or a sandbox without a local Supabase. `database.types.ts` is generated and must not be hand-edited. Once regenerated, the casts, the `select("*")` widening, and the now-dormant pre-apply fallbacks (`isMissingMemberIdColumn`, the missing-table `[]` degradations) can all be retired with the compiler as the safety net.
...
3. **[RESOLVED in code — needs one ops step] Background function wire secret.** It authenticated dispatches by comparing `x-internal-secret` to `SUPABASE_SERVICE_ROLE_KEY` with `!==`, so the RLS-bypassing database master key travelled as a request header on every generation, translation and workout dispatch. **Fix:** `INTERNAL_FN_SECRET` (`getInternalFnSecret()` in `env.ts`, sent from all three dispatch sites) + constant-time comparison (`sha256` then `timingSafeEqual`, so unequal lengths don't leak through an early return). The service key is still ACCEPTED during rollout and a `console.warn` fires when it is used, so no coordinated deploy is needed and the legacy branch can be removed once that warning stops. The service key's only remaining job in the function is PostgREST auth. **OPS: set `INTERNAL_FN_SECRET` (random 32 bytes, base64) in Netlify — unscoped, so BOTH the app and the background function see it — and in local `.env.local`.** Until then behavior is unchanged.
4. **`sharp` and `postcss` advisories are pinned inside next** and need a major bump to clear.
5. **[S1 · CONFIRMED — fixed] Tier-capped runs could never translate for the housekeeper (dev path).** The meal-completion block exists twice — `runMealPlanGeneration` (dev, inline, SDK) and the background function (prod, SDK-free fetch) — and they had drifted: the background copy narrowed translation-readiness to a capped run's `limitMemberIds`, the engine copy did not. So on a tier-capped family the deferred members kept `hasPendingGeneration` true forever and the housekeeper translation never started. Production was unaffected (it runs the background copy); the inline path is dev-only. **Fix:** `runMealPlanGeneration` takes `limitMemberIds` and both runners now share four pure decision helpers (`resolveHousekeeperLocale`, `planNeedsTranslation`, `translationMemberIds`, `partialPlanNote`), unit-tested. `resolveHousekeeperLocale` lives in `schema.ts` beside `LOCALE_CODES` and now serves all FOUR sites that decided it — context building (`buildContext` + the function's fetch mirror) and completion (both runners).

6. **Structural merge of the two meal runners — investigated, deliberately NOT done.** The proposal was to route the background function through `runMealPlanGeneration` via `makeFetchSupabase` (the move already proven for `runWorkoutPlanGeneration`). It does not transfer: that adapter implements only `from(t).update(v).eq(c, val)`, while `runMealPlanGeneration` also needs two SELECT chains (`.select().eq().eq().limit().returns()` for the housekeeper read and the members read). Merging would mean building a PostgREST select-chain query builder into the most critical path in the product, which has no route-level tests. The remaining duplication is IO only (SDK vs fetch), and it is now decision-free — every judgment the two runners make is shared and tested, so they can state the same conclusions differently but cannot reach different ones. Revisit only if the adapter grows select support for another reason.

7. Duplication left in place: the pre-generation `limitMemberIds` filter is applied in both runners on purpose (dispatch narrows `context.family_members` in place for the inline path; the background function rebuilds context from the DB and must re-apply it).
