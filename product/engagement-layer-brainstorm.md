...
> **Owner override A (07/2026) — leaderboard:** For the `«موسم بيتنا»` leaderboard (`FamilySeasonCard`) the owner directed a competitive ranked shape, which **supersedes guardrails 5 (no shame/last-place) and 8 (rewards attach to rituals not adherence counts) on that surface only** — per-member ranks, a gold #1 «winner», weekly %. Housekeeper (3) never on it; hidden for solo households (7). Children DO rank there (a separate later directive). See `product/family-engagement-research-and-plan.md` (top note).
>
> **Owner override B (07/2026) — child weight tracking:** The owner directed including CHILDREN in the private weight journey («رحلتك الخاصة»/«الوزن والمتابعة»), **partially superseding guardrail 1's "no weight" for children** — with the child-safety trade-offs surfaced. Kept NARROW: children get a PRIVATE, neutral weight-over-time record ONLY. **Held firm even under the directive: NO body photos of minors** (blocked in form + page + server `logBodyWeight`) and **a child's weight/goal never appears on any SHARED surface** (`isGoalCelebrationEligibleMember` keeps children off the «تحقّق الهدف» celebration; recap weight-delta is mom-only). The housekeeper is still never tracked. Split lives in `apps/app/src/lib/engagement/eligibility.ts`; see `CLAUDE.md` for the mechanics.

1. **Children:** no goals, no calories, ~~no weight~~ (weight tracking now allowed as a PRIVATE record per owner override B above — no photos, never on a shared surface), no consumption tracking, no sibling comparison — participation and food-curiosity only, self-referential. (Extends the stance already in code.)
