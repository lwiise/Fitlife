...
الحقول العربية تبقى كما هي (هي المرجع للأم).`
  ...
  const sharedRule = isSolo
    ...
    : "الوجبة تُطبخ مرة واحدة وتُقسَّم. عند تطابق اسم الطبق بين أفراد يناسبهم نفس الطبق: ضعي shared_recipe=true، واكتبي وصفة واحدة في ingredients مقيسة لكمية المجموعة كاملة، واذكري الوزن النهائي الإجمالي في batch_finished_weight_g. وفي per_member_portions لكل فرد مشارك اذكري حصته portion_grams (بالجرام) و portion_percentage (نسبة من الإجمالي)، ومجموع النسب ≈ 100. استخدمي حقلَي ingredients و notes_ar داخل per_member_portions فقط لإضافة حقيقية لفرد بعينه — لا لإعادة كتابة الوصفة. مَن لا يناسبه الطبق (حساسية أو ماكروز لا يحققها) أعطيه وصفة خاصة لتلك الوجبة. للأطفال: حصص مناسبة بدون معادلات سعرات.";
