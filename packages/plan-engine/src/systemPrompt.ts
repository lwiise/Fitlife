...
الحقول العربية تبقى كما هي (هي المرجع للأم).`
  ...
  const sharedRule = isSolo
    ...
    : "الوجبة تُطبخ مرة واحدة وتُقسَّم على المشاركين. شاركي الطبق بين الأفراد فقط حين يناسبهم جميعاً فعلاً (لا حساسية متعارضة، ولا قيد غذائي أو كره قوي، ولا اختلاف في الماكروز/حالة طبية يمنع ذلك). مَن لا يناسبه الطبق أعطيه وصفة فردية خاصة لتلك الوجبة ولا تُجبريه على المشاركة. الوجبة التي يأكلها فرد واحد فقط ليست مشتركة — لا تضعي لها حقول المشاركة. " +
      "عند المشاركة (فردان أو أكثر): ضعي shared_recipe=true، واكتبي وصفة واحدة في ingredients بمقادير **إجمالية لكل المشاركين مجتمعين** — أي مجموع كميات الجميع للقِدر كامله، وليست حصة شخص واحد (مثال: إذا احتاج ثلاثة أفراد 60 و80 و100 جم دجاج، اكتبي «دجاج 240 جم»). طبّقي نفس المنطق على كل المكونات. اذكري الوزن النهائي الإجمالي في batch_finished_weight_g (= مجموع الحصص). وفي per_member_portions اذكري **كل** فرد مشارك بحصته portion_grams (بالجرام) و portion_percentage (نسبة من الإجمالي)، ومجموع النسب ≈ 100. استخدمي حقلَي ingredients و notes_ar داخل per_member_portions فقط لإضافة حقيقية لفرد بعينه — لا لإعادة كتابة الوصفة. للأطفال: حصص مناسبة بدون معادلات سعرات.";
...
type DaySlice = {
  ...
  members: Array<{
    ...
    meals: Array<{
      ...
      shared_recipe?: boolean;                 // true = طُبخت مرة واحدة وقُسِّمت
      batch_finished_weight_g?: number;        // الوزن النهائي الإجمالي للطبق المشترك
      per_member_portions?: Array<{ member_id: string; portion_grams?: number; portion_percentage?: number; ingredients?: Array<{ name_ar: string; amount: number; amount_min?: number; amount_max?: number; unit: string }>; notes_ar?: string }>;
