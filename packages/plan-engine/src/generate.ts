...
export async function generateMealPlan(params: {
  ...
  // TEMP diagnostic — only when there's exercise intent, so meal-only plans stay clean.
  const exerciseIntent =
    !!context.mom.exercise_profile ||
    context.family_members.some((m) => !!m.exercise_profile);
  const exerciseDebug = exerciseIntent
    ? {
  ...
  return {
    plan: {
      ...
      ...(exerciseDebug ? { exercise_debug: exerciseDebug } : {}),
    },
    usage: {
